...
This module computes all kernel approximation features (random and Nystrom).
...
class RandomFeaturesSampler(ABC, BaseEstimator, TransformerMixin):
    ...
    def fit_transform(
        ...
        X_prime: Optional[
            np.ndarray
        ] = None,  # Added to ensure compatibility with sklearn
    ) -> np.ndarray:
...
class RandomFeaturesSamplerRBF(RandomFeaturesSampler):
    ...
    def __init__(self, sigma: float = 1.0, n_random_features: int = 100) -> None:
        ...
        super().__init__(
            n_random_features
        )  # Initializes the number of random features of the inherited class
...
class RandomFeaturesSamplerMatern(RandomFeaturesSampler):
    ...
    def __init__(
        self, scale: float = 1.0, nu: float = 1.5, n_random_features: int = 100
    ) -> None:
...
class NystroemFeaturesSampler(BaseEstimator, TransformerMixin):
    ...
    def fit(
        ...
        if self.n_random_features > n_instances:
            ...
            warnings.warn(
                "n_random_features > n_samples, n_random_features was set to n_instances."
            )
        else:
    ...
    # This function computes correctly the approximate kernel matrix, when necessary
...
def demo_kernel_approximation_error_evolution(
    ...
    if isinstance(features_sampler, NystroemFeaturesSampler):
        plt.plot(
            features_range,
            1 / np.power(features_range, 1.25),
