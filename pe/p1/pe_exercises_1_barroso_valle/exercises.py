...
This file contains all the methods used in the simulations.
...
def plot_poisson_erlang(lambda_rate, M, N, t0, t1):
    ...
    >>> import arrival_process_simulation as arrival
    >>> from scipy import stats, integrate, special
    >>> lambda_rate, T, M, N = 10, 5, 50000, N = numpy.ndarray([1, 2, 5, 10])
    >>> t0, t1 = 0., 7.
...
def simulate_wiener(t0, T, M, N):
    """
    Simulation of standard Wiener process in [t0, t0+T]
    ...
    return t, np.dot(Z, np.sqrt(t).reshape(1, N+1))
