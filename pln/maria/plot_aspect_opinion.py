...
def compute_cloud_opinion_word(data_aspect, aspect_name):
    ...
    wordcloud_pos = WordCloud(stopwords=stopwords, background_color="white", width=800, height=400).generate(text_pos)
    wordcloud_neg = WordCloud(stopwords=stopwords, background_color="white", width=800, height=400).generate(text_neg)
...
def plot_pie(df_aspect_polarity):
    ...
    sizes_polarity = list(itertools.chain(*zip(df_aspect_polarity.neg.values, df_aspect_polarity.pos.values)))

    plt.pie(sizes_aspect, labels=labels_aspect, startangle=90, frame=True,  textprops={'fontsize': 25}, autopct='%.2f%%',  pctdistance=0.85)
    plt.pie(sizes_polarity,colors=colors_polarity,radius=0.75,startangle=90)
